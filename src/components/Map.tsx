  ...
  useEffect(() => {
    ...
    if (slots.length > 0) {
        ...
        mapRef.current.fitBounds(bounds, {
            padding: 60,
            maxZoom: 14,
            duration: 1000, // Sanfter Übergang beim Stadtwechsel
        });
    }
    ...
    slots.forEach((slot) => {
        ...
        titleEl.textContent = slot.name + " " + (slot.frequency || '');
        infoEl.appendChild(titleEl);

        const subTypeEl = document.createElement('p');
        subTypeEl.className = 'text-xs text-gray-600';
        subTypeEl.textContent = slot.subType || '';
        infoEl.appendChild(subTypeEl);

        const addressEl = document.createElement('p');
        addressEl.className = 'text-xs text-gray-400';
        addressEl.textContent = slot.address || '';
